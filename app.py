...
# --- justo después de los imports al inicio de app.py ---
import os, requests, streamlit as st

# Cargar secrets a env
for k in ["VIYA_URL","MAS_MODULE_ID","BEARER_TOKEN","CA_PEM"]:
    if k in st.secrets:
        os.environ[k] = st.secrets[k]

# Si nos pasaron una CA como texto, guardarla y usarla
if os.getenv("CA_PEM"):
    with open("/tmp/viya.pem","w") as f:
        f.write(os.environ["CA_PEM"])
    os.environ["VIYA_CA_BUNDLE"] = "/tmp/viya.pem"

# NO auto-llames a SASLogon aquí. Solo usa BEARER_TOKEN si está:
if not os.getenv("BEARER_TOKEN"):
    st.warning("No BEARER_TOKEN configurado. Cárgalo en Secrets o exporta la variable de entorno en el host.")
